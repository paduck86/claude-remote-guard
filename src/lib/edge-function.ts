...
export const EDGE_FUNCTION_CODE = `// Supabase Edge Function for Slack Interactive Callbacks
...
export function createEdgeFunctionFiles(targetDir: string): CreateEdgeFunctionResult {
  try {
    const functionsDir = path.join(targetDir, 'supabase', 'functions', 'slack-callback');
    ...
    fs.writeFileSync(filePath, EDGE_FUNCTION_CODE, 'utf-8');

    return {
      success: true,
      path: path.join('supabase', 'functions', 'slack-callback'),
    };
  } catch (error) {
    return {
      success: false,
